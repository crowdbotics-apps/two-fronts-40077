...
export default function PlaidLink() {
  ...
  useEffect(() => {
    if(publisherData){
      publisherDataDecoded = Buffer.from(publisherData, 'base64').toString('ascii')
  ...
  useEffect(() => {
    ...
    if(!isToken) {
      nav(`/${APPCONFIG.routes.login}`, {state: {redirectUri: `/${APPCONFIG.routes.connectYourBank}`}})
      return
    }
    getToken();
  }, [])
      ...
      <PlaidContainer>
        <Container className='px-95 text-center mt-5'>
          ...
          <Container className='w-75 mt-5 text-start'>
            ...
            <p className='m-bottom-11 font-normal-discription-16 p-left-36'>All bank communications are encrypted end to end.</p>
            <p className='m-bottom-11 font-extra-bold-18'>
              <img src = {svg.Checkmark} alt = '' />
              <span className='p-left-12'>Private</span>
            </p>
            <p className='m-bottom-11 font-normal-discription-16 p-left-36'>We will never see or save your bank account information</p>
          </Container>
        </Container>
        ...
        <Container className='fix-bottom px-95'>
          ...
          <Button variant='secondary' className='rounded-pill font-bold-14 text-white plaid-btn'>
            Add funds later
          </Button>
        </Container>
      </PlaidContainer>
